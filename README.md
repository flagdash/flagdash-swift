...
```swift
let replay = InteractionReplay(sdkKey: replayKey, release: appVersion)
await replay.observeLifecycle()
if await replay.start() {
    await replay.screen("Checkout")
    await replay.interaction("buy_tapped", screen: "Checkout")
    _ = await replay.stop()
}
```

iOS replay is an explicit interaction timeline, not a screen recording. It never captures pixels, view trees, or input values. Sensitive property keys are redacted; use a key scoped only to `replays:write`.
...
```swift
let release = await client.aiConfigRelease("support-agent", userID: "usr_123")
```
