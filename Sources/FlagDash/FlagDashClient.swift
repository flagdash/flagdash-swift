...
public actor FlagDashClient {
    ...
    public func aiConfigRelease(_ key: String, userID: String = "anonymous") async -> [String: JSONValue]? {
        do {
            let envelope: AIEnvelope = try await get("/ai-config-releases/\(segment(key))", context: EvaluationContext(userID: userID))
            return envelope.aiConfig
        } catch { return nil }
    }
